...
namespace FairyGUI {
    ...
    public class GComponent : GObject {
        ...
        public void RemoveController(Controller c) {
            var index = _controllers.IndexOf(c);
            if (index == -1) {
        ...
        private void BuildNativeDisplayList() {
            ...
            switch (_childrenRenderOrder) {
                ...
                case ChildrenRenderOrder.Arch: {
                    var apex = Mathf.Clamp(_apexIndex, 0, cnt);
        ...
        public bool IsChildInView(GObject child) {
            if (scrollPane != null) {
                ...
                return child.x + child.width >= 0 && child.x <= width && child.y + child.height >= 0 && child.y <= height;
            } else {
                return true;
            }
        }
        ...
        private void UpdateClipRect() {
            if (scrollPane == null) {
                var w = width - (_margin.left + _margin.right);
                var h = height - (_margin.top + _margin.bottom);
                rootContainer.clipRect = new Rect(_margin.left, _margin.top, w, h);
            } else {
                rootContainer.clipRect = new Rect(0, 0, width, height);
            }
        }
        ...
        override protected void HandleGrayedChanged() {
            var cc = GetController("grayed");
            if (cc != null) {
                cc.selectedIndex = grayed ? 1 : 0;
            } else {
        ...
        public float viewWidth {
            ...
            set {
                if (scrollPane != null) {
                    ...
                    width = value + _margin.left + _margin.right;
                }
            }
        }
        ...
        public float viewHeight {
            get {
                if (scrollPane != null) {
                    ...
                    return height - _margin.top - _margin.bottom;
                }
            }
            ...
            set {
                if (scrollPane != null) {
                    ...
                    height = value + _margin.top + _margin.bottom;
                }
            }
        }
        ...
        */
        ...
        internal void ChildSortingOrderChanged(GObject child, int oldValue, int newValue) {
            if (newValue == 0) {
                ...
                var oldIndex = _children.IndexOf(child);
                var index = GetInsertPosForSortingChild(child);
                if (oldIndex < index) {
        ...
        internal void ConstructFromResource(List<GObject> objectPool, int poolIndex) {
            ...
            for (var i = 0; i < childCount; i++) {
                ...
                if (child.displayObject != null) {
                    child.displayObject.cachedTransform.SetParent(displayObject.cachedTransform, false);
                }
...

            {
                ...
                var i2 = buffer.ReadInt();
                if (hitTestId != null) {
                    var pi = contentItem.owner.GetItem(hitTestId);
                    ...
                    rootContainer.hitArea = new ShapeHitTest(GetChildAt(i2).displayObject);
                }
            }
            ...
            if (buffer.version >= 5) {
                ...
                if (!string.IsNullOrEmpty(str)) {
                    onAddedToStage.Add(() => __playSound(str, 1));
                }

                var str2 = buffer.ReadS();
                if (!string.IsNullOrEmpty(str2)) {
                    onRemovedFromStage.Add(() => __playSound(str2, 1));
                }
            }
            ...
            if (_transitions.Count > 0) {
                ...
                onRemovedFromStage.Add(__removedFromStage);
            }
        ...
        public override void Setup_AfterAdd(ByteBuffer buffer, int beginPos) {
            ...
            for (var i = 0; i < cnt; i++) {
                var cc = GetController(buffer.ReadS());
        ...
        private void __playSound(string soundRes, float volumeScale) {
            var sound = UIPackage.GetItemAssetByURL(soundRes) as NAudioClip;

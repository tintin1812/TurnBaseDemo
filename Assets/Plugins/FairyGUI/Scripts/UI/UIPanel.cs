...
namespace FairyGUI
{
    ...
    public class UIPanel : MonoBehaviour, EMRenderTarget
    {
        ...
        [SerializeField]
        private bool setNativeChildrenOrder = false;

        [NonSerialized]
        private int screenSizeVer;
        [NonSerialized]
        private Rect uiBounds; //Track bounds even when UI is not created, edit mode
        ...
        private void CreateContainer()
        {
            if (!Application.isPlaying)
            {
                ...
                while (cnt > 0)
                {
                    var go = t.GetChild(cnt - 1).gameObject;
            ...
            container = new Container(gameObject);
            ...
            if (Application.isPlaying)
            {
                ...
                if (hitTestMode == HitTestMode.Raycast)
                {
                    var hitArea = new ColliderHitTest();
                    hitArea.collider = gameObject.AddComponent<BoxCollider>();
                ...
                if (setNativeChildrenOrder)
                {
                    ...
                    container.onUpdate += () =>
                    {
                        var cnt = _renders.Count;
                        var sv = UpdateContext.current.renderingOrder++;
                        for (var i = 0; i < cnt; i++)
                        {
                            var r = _renders[i];
                            if (r != null) {
        ...
        public void CacheNativeChildrenRenderers()
        {
            ...
            var t = container.cachedTransform;
            var cnt = t.childCount;
            for (var i = 0; i < cnt; i++)
            {
        ...
        {
            var hitArea = container.hitArea as ColliderHitTest;
            if (hitArea != null)
            {
        ...
        private void HandleScreenSizeChanged()
        {
            ...
            if (container != null)
            {
                var cam = container.GetRenderCamera();
                ...
                if (container.renderMode != RenderMode.WorldSpace)
                {
                    var sc = cam.GetComponent<StageCamera>();
                    if (sc == null) {
                        sc = StageCamera.main.GetComponent<StageCamera>();
                    }
                    container.scale = new Vector2(sc.unitsPerPixel * UIContentScaler.scaleFactor, sc.unitsPerPixel * UIContentScaler.scaleFactor);
                }
            }
        ...
        public void ApplyModifiedProperties(bool sortingOrderChanged, bool fitScreenChanged)
        {
            ...
            if (fitScreenChanged && fitScreen == FitScreen.None)
            {
                if (_ui != null) {
        ...
        public Vector3 GetUIWorldPosition()
        {
            if (_ui != null) {
                ...
                return container.cachedTransform.TransformPoint(uiBounds.position);
            }
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                Gizmos.matrix = container.cachedTransform.localToWorldMatrix;

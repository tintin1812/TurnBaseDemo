            ...
            get {
                if (_virtual) {
                    ...
                    for (var i = 0; i < cnt; i++) {
                        ...
                        if ((ii.obj is GButton) && ((GButton) ii.obj).selected || ii.obj == null && ii.selected) {
                            if (_loop) {
                                return i % _numItems;
                            } else {
                                return i;
        ...
        public void ScrollToView(int index, bool ani, bool setFirst) {
            if (_virtual) {
                ...
                if (_layout == ListLayoutType.SingleColumn || _layout == ListLayoutType.FlowHorizontal) {
                    ...
                    rect = new Rect(page * viewWidth + (index % _curLineItemCount) * (ii.size.x + _columnGap), (index / _curLineItemCount) % _curLineItemCount2 * (ii.size.y + _lineGap), ii.size.x, ii.size.y);
                }
            ...
            if (layoutChanged) {
                if (_layout == ListLayoutType.SingleColumn || _layout == ListLayoutType.SingleRow) {
                    ...
                    if (_columnCount > 0) {
                        ...
                        _curLineItemCount = Mathf.FloorToInt((scrollPane.viewWidth + _columnGap) / (_itemSize.x + _columnGap));
                    ...
                    if (_lineCount > 0) {
                        ...
                        _curLineItemCount = Mathf.FloorToInt((scrollPane.viewHeight + _lineGap) / (_itemSize.y + _lineGap));
                        if (_curLineItemCount <= 0) {
                            _curLineItemCount = 1;
                        }
                    }
                } else //pagination
                {
                    ...
                    if (_lineCount > 0) {
                        ...
                        _curLineItemCount2 = Mathf.FloorToInt((scrollPane.viewHeight + _lineGap) / (_itemSize.y + _lineGap));
            ...
            if (_layout == ListLayoutType.SingleColumn) {
                ...
                if (_autoResizeItem && _lineCount > 0) {
                    ...
                    for (i = 0; i < cnt; i++) {
                        ...
                        if (_lineCount != 0 && j >= _lineCount || _lineCount == 0 && curY + child.height > viewHeight && maxWidth != 0) {
                            curY = 0;
                            curX += Mathf.CeilToInt(maxWidth) + _columnGap;
                            maxWidth = 0;
                            j = 0;
            ...
            } else //pagination
            {
                ...
                if (_autoResizeItem && _columnCount > 0) {
                    ...
                    for (i = 0; i < cnt; i++) {
                        ...
                        if (j == 0 && (_lineCount != 0 && k >= _lineCount || _lineCount == 0 && curY + (_lineCount > 0 ? eachHeight : child.height) > viewHeight)) {
                            //new page
                            page++;
                            curY = 0;
                            k = 0;
                    ...
                    for (i = 0; i < cnt; i++) {
                        ...
                        if (_columnCount != 0 && j >= _columnCount || _columnCount == 0 && curX + child.width > viewWidth && maxHeight != 0) {
                            ...
                            if (_lineCount != 0 && k >= _lineCount || _lineCount == 0 && curY + child.height > viewHeight && maxWidth != 0) //new page
                            {
                                page++;
                                curY = 0;
                                k = 0;

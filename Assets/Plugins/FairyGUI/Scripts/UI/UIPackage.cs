...
namespace FairyGUI {
    ...
    public class UIPackage {
...
#if UNITY_EDITOR
        private static LoadResource _loadFromAssetsPath = (string name, string extension, Type type, out DestroyMethod destroyMethod) => {
        ...
        private static LoadResource _loadFromResourcesPath = (string name, string extension, Type type, out DestroyMethod destroyMethod) => {
            destroyMethod = DestroyMethod.Unload;
            return Resources.Load(name, type);
        };
        ...
        private bool LoadPackage(ByteBuffer buffer, string assetNamePrefix) {
            ...
            if (_packageInstById.TryGetValue(id, out existingPkg)) {
                ...
                if (Application.isEditor) {
                    ...
                    } else if (existingPkg._loadFunc == _loadFromResourcesPath && _loadFunc == _loadFromResourcesPath && _assetPath.Length < existingPkg._assetPath.Length) //both in resources path, pefer short path
                    {
                        existingPkg.Dispose(); //replace the existing
        ...
        private void LoadMovieClip(PackageItem item) {
            ...
            for (var i = 0; i < frameCount; i++) {
                ...
                if (spriteId != null && _sprites.TryGetValue(spriteId, out sprite)) {
                    frame.texture = new NTexture((NTexture) GetItemAsset(sprite.atlas), sprite.rect, sprite.rotated, new Vector2(item.width, item.height), frameRect.position);
                }
                item.frames[i] = frame;
            ...
            for (var i = 0; i < cnt; i++) {
                ...
                if (ttf) {
                    if (mainSprite.rotated) {
                        bg.uv[0] = new Vector2((float) (by + bgHeight + mainSprite.rect.x) * texScaleX, 1 - (float) (mainSprite.rect.yMax - bx) * texScaleY);
                        ...
                        bg.uv[0] = new Vector2((float) (bx + mainSprite.rect.x) * texScaleX, 1 - (float) (by + bgHeight + mainSprite.rect.y) * texScaleY);

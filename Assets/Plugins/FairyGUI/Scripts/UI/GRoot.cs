...
namespace FairyGUI
{
    ...
    public class GRoot : GComponent
    {
        ...
        public GRoot()
        {
            name = rootContainer.name = rootContainer.gameObject.name = "GRoot";
            opaque = false;
        ...
        private void CreateModalLayer()
        {
            ...
            _modalLayer.DrawRect(width, height, 0, Color.white, UIConfig.modalLayerColor);
        ...
        public void ShowPopup(GObject popup, GObject target, PopupDirection dir, bool closeUntilUpEvent)
        {
            ...
            {
                var k = _popupStack.IndexOf(popup);
                if (k != -1)
                {
                    for (var i = _popupStack.Count - 1; i >= k; i--)
                    {
                        var last = _popupStack.Count - 1;
                        var obj = _popupStack[last];
                        ClosePopup(obj);
            ...
            var pos = GetPoupPosition(popup, target, dir);
            popup.xy = pos;
        }
        ...
        public Vector2 GetPoupPosition(GObject popup, GObject target, PopupDirection dir)
        {
            ...
            {
                pos = GlobalToLocal(Stage.inst.touchPosition);
            }
            float xx, yy;
            xx = pos.x;
            if (xx + popup.width > width) {
                xx = xx + size.x - popup.width;
            }
            yy = pos.y + size.y;
            if ((dir == PopupDirection.Auto && yy + popup.height > height)
        ...
        public void HidePopup(GObject popup)
        {
            ...
            {
                foreach (var obj in _popupStack) {
                    ClosePopup(obj);
                }
                _popupStack.Clear();
        ...
        private void __showTooltipsWin(object param)
        {
            ...
            var xx = Stage.inst.touchPosition.x + 10;
            var yy = Stage.inst.touchPosition.y + 20;

            var pt = GlobalToLocal(new Vector2(xx, yy));
            xx = pt.x;
            yy = pt.y;

            if (xx + _tooltipWin.width > width) {
                xx = xx - _tooltipWin.width;
            }
            if (yy + _tooltipWin.height > height)
            {
        ...
        public GObject focus
        {
            get
            {
                var obj = DisplayObjectToGObject(Stage.inst.focus);
        ...
        private void CheckPopups(bool touchBegin)
        {
            ...
            if (_popupStack.Count > 0)
            {
                var mc = Stage.inst.touchTarget as DisplayObject;
                var handled = false;
                while (mc != Stage.inst && mc != null)
                {
                    if (mc.gOwner != null)
                    {
                        var k = _popupStack.IndexOf(mc.gOwner);
                        if (k != -1)
                        {
                            for (var i = _popupStack.Count - 1; i > k; i--)
                            {

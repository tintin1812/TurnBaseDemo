...
#pragma warning restore 0649
        ...
        public Stage() : base() {
            ...
            var isOSX = Application.platform == RuntimePlatform.OSXPlayer || Application.platform == RuntimePlatform.OSXEditor;
            if (Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor || isOSX) {
                touchScreen = false;
            } else {
            ...
            var engine = GameObject.FindObjectOfType<StageEngine>();
            if (engine != null) {
        ...
        internal void HandleScreenSizeChanged(int screenWidth, int screenHeight, float unitsPerPixel) {
            ...
            if (!DispatchEvent("onStageResized", null)) {
                var scaler = this.gameObject.GetComponent<UIContentScaler>();
                scaler.ApplyChange();
        ...
        public void AddTouchMonitor(int touchId, EventDispatcher target) {
            ...
            for (var j = 0; j < 5; j++) {
                touch = _touches[j];
                if (touchId == -1 && touch.touchId != -1 || touchId != -1 && touch.touchId == touchId) {
                    break;
                }
            }
    ...
    class TouchInfo {
        ...
        public void End() {
            ...
            if (downTargets.Count == 0 || clickCancelled || Mathf.Abs(x - downX) > Stage._clickTestThreshold || Mathf.Abs(y - downY) > Stage._clickTestThreshold) {
                clickCancelled = true;
                lastClickTime = 0;
                clickCount = 1;
            } else {
                if (Time.unscaledTime - lastClickTime < 0.35f && Mathf.Abs(x - lastClickX) < Stage._clickTestThreshold && Mathf.Abs(y - lastClickY) < Stage._clickTestThreshold && lastClickButton == button) {
                    if (clickCount == 2)
                        clickCount = 1;
                    else
                        clickCount++;

...
namespace FairyGUI
{
    ...
    public class DisplayObject : EventDispatcher
    {
        ...
        public void EnterPaintingMode(int requestorId, Margin? extend, float scale)
        {
            ...
            if (first)
            {
                if (_paintingInfo == null)
                {
                    _paintingInfo = new PaintingInfo() { captureDelegate = Capture, scale = 1 };
                }
        ...
        virtual public Rect GetBounds(DisplayObject targetSpace)
        {
            ...
            {
                return new Rect(cachedTransform.localPosition.x, -cachedTransform.localPosition.y, _contentRect.width * cachedTransform.localScale.x, _contentRect.height * cachedTransform.localScale.y);
            }
            else
        ...
        public Rect TransformRect(Rect rect, DisplayObject targetSpace)
        {
            ...
            if (targetSpace == parent && _rotation.z == 0) // optimization
            {
                ...
                return new Rect((this.x + rect.x) * vec.x, (this.y + rect.y) * vec.y, rect.width * vec.x, rect.height * vec.y);
            }
            else
            {
    ...
    public class DisplayObjectInfo : MonoBehaviour
    {
        ...
        /// 
        /// /// </summary>
        [System.NonSerialized] public DisplayObject displayObject;
        ...
        {
            displayObject.renderingOrder = order;
        }
    }
}